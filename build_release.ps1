# Build the distributable mod zip: the game-folder layout the mod lives in (Data\Lua\baba_is_read.lua,
# Data\Lua\baba_is_read\** with babaisread.dll and prism.dll under bin\), freshly compiled, as
# releases\BabaIsRead-v<version>.zip. The zip root IS the game folder, so the installer (and a manual
# user) extracts it straight into the game dir. The version is the mod's own
# (lua\baba_is_read\version.lua), which is also the release tag.
#
# Not shipped: the dev server's scratch (lua\baba_is_read\cmd\, gitignored) and anything else not
# checked in.
#
...
$versionFile = Join-Path $scriptDir "lua\baba_is_read\version.lua"
...
$bootstrap = Join-Path $luaDir "baba_is_read.lua"
$modSrcDir = Join-Path $luaDir "baba_is_read"
$bridgeDll = Join-Path $scriptDir "build\babaisread.dll"
$prismDll = Join-Path $scriptDir "third_party\prism\prism.dll"
$zipPath = Join-Path $releaseDir "BabaIsRead-v$version.zip"

# Directories under lua\baba_is_read\ that are never part of a release.
$excludedDirs = @("cmd")
...
try {
    ...
    # The same layout build.ps1 deploys: the bootstrap in Data\Lua, the modules under baba_is_read\.
    $stageLuaDir = Join-Path $stageDir "Data\Lua"
    $stageModDir = Join-Path $stageLuaDir "baba_is_read"
    ...
    foreach ($required in @("Data\Lua\baba_is_read.lua", "Data\Lua\baba_is_read\main.lua",
            "Data\Lua\baba_is_read\bin\babaisread.dll", "Data\Lua\baba_is_read\bin\prism.dll")) {
