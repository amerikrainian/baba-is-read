...
use super::paths::{GAME_DATA_MARKER, GAME_EXES, GAME_FOLDERS};

/// Where a detected install came from, recorded in the manifest. Only the Steam
/// install is looked for; an itch.io or Humble copy is reached through Browse (Manual).
/// A new storefront adds a variant here plus its own candidates source in `game_candidates`.
...
pub fn missing_marker(path: &Path) -> Option<String> {
    ...
    if !path.join(GAME_DATA_MARKER).exists() {
        return Some(GAME_DATA_MARKER.to_string());
    }
    None
}
...
pub fn game_candidates() -> Vec<GameInstall> {
    ...
    // BABA_DIR is the same override the mod's own build (build.ps1, tools/dev.py) uses for the game dir.
    if let Ok(value) = std::env::var("BABA_DIR") {
        add_candidate(
            &mut result,
            &mut seen,
            PathBuf::from(value),
...
mod tests {
    ...
    fn write_markers(dir: &Path) {
        fs::write(dir.join("Baba Is You.exe"), "").unwrap();
        let data = dir.join("Data");
        fs::create_dir_all(&data).unwrap();
        fs::write(data.join("modsupport.lua"), "").unwrap();
    }
    ...
    #[test]
    fn rejects_dir_without_game_data() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("Baba Is You.exe"), "").unwrap();
        assert!(!validate_game_dir(dir.path()));
        assert_eq!(
            missing_marker(dir.path()),
            Some(GAME_DATA_MARKER.to_string())
        );
    }
    ...
    fn missing_marker_names_the_exe_first() {
        ...
        assert_eq!(
            missing_marker(dir.path()),
            Some("Baba Is You.exe".to_string())
        );
    }
}
