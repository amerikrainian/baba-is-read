...
/// (e.g. a file a manual unzip left read-only).
...
mod tests {
    ...
    fn write_vanilla_game(dir: &Path) {
        fs::write(dir.join("Baba Is You.exe"), "game").unwrap();
        fs::create_dir_all(dir.join("Data")).unwrap();
        fs::write(dir.join(paths::GAME_DATA_MARKER), "-- hooks").unwrap();
        fs::create_dir_all(paths::mod_root(dir)).unwrap();
    }
    ...
    fn classifies_unmanaged_by_file_presence_only() {
        ...
        let bootstrap = dir.path().join(paths::BOOTSTRAP_REL);
        fs::create_dir_all(bootstrap.parent().unwrap()).unwrap();
        fs::write(bootstrap, "-- a dev deploy").unwrap();
        assert!(matches!(
    ...
    fn invalid_manifest_with_files_is_damaged_state() {
        ...
        let bridge = dir.path().join(paths::BRIDGE_REL);
    ...
    fn fresh_install_then_uninstall_leaves_game_dir_clean() {
        let dir = tempfile::tempdir().unwrap();
        write_vanilla_game(dir.path());
        let zip_path = dir.path().join("release.zip");
        create_zip(
            &zip_path,
            &[
                (paths::BOOTSTRAP_REL, "bootstrap"),
                (MAIN_REL, "main"),
                (LANG_REL, "strings"),
                (paths::BRIDGE_REL, "bridge"),
                (PRISM_REL, "speech"),
            ],
        );
        ...
        assert!(dir.path().join(paths::BOOTSTRAP_REL).exists());
        assert!(dir.path().join(PRISM_REL).exists());
        assert!(matches!(
            classify_install(dir.path()),
        ...
        // The game's own files, and its empty Data/Lua, are untouched.
        assert!(dir.path().join("Baba Is You.exe").exists());
        assert!(dir.path().join(paths::GAME_DATA_MARKER).exists());
        assert!(paths::mod_root(dir.path()).is_dir());
        assert_eq!(fs::read_dir(paths::mod_root(dir.path())).unwrap().count(), 0);
    ...
    fn overwritten_file_is_backed_up_and_restored_on_uninstall() {
        let dir = tempfile::tempdir().unwrap();
        // A pre-existing bootstrap (say, a dev deploy or a manual unzip) must survive a
        // managed install + uninstall round trip: that is what "repair" over an unmanaged
        // install does.
        let bootstrap = dir.path().join(paths::BOOTSTRAP_REL);
        fs::create_dir_all(bootstrap.parent().unwrap()).unwrap();
        fs::write(&bootstrap, "original bootstrap").unwrap();

        let zip_path = dir.path().join("release.zip");
        create_zip(
            &zip_path,
            &[(paths::BOOTSTRAP_REL, "new bootstrap"), (MAIN_REL, "main")],
        );
        ...
        let backup_rel = manifest
            .backups
            .get(paths::BOOTSTRAP_REL)
            .expect("existing file should be backed up")
            .clone();
        assert!(backup_rel.starts_with(paths::BACKUPS_REL));
        assert!(dir.path().join(&backup_rel).exists());
        assert_eq!(fs::read_to_string(&bootstrap).unwrap(), "new bootstrap");

        uninstall::uninstall(dir.path(), &manifest).unwrap();

        assert_eq!(fs::read_to_string(&bootstrap).unwrap(), "original bootstrap");
    ...
    fn overwrites_and_uninstalls_read_only_files() {
        let dir = tempfile::tempdir().unwrap();
        // A read-only pre-existing file must not block install or uninstall.
        let existing = dir.path().join(LANG_REL);
        fs::create_dir_all(existing.parent().unwrap()).unwrap();
        fs::write(&existing, "old strings").unwrap();
        let mut perms = fs::metadata(&existing).unwrap().permissions();
        perms.set_readonly(true);
        fs::set_permissions(&existing, perms).unwrap();

        let zip_path = dir.path().join("release.zip");
        create_zip(&zip_path, &[(MAIN_REL, "main"), (LANG_REL, "new strings")]);
        ...
        assert_eq!(fs::read_to_string(&existing).unwrap(), "new strings");
        let backup_rel = manifest.backups.get(LANG_REL).unwrap();
        assert_eq!(
        ...
        assert_eq!(fs::read_to_string(&existing).unwrap(), "old strings");
        assert!(!dir.path().join(paths::BACKUPS_REL).exists());
    }
    ...
    fn orphaned_file_with_backup_is_restored_on_upgrade() {
        let dir = tempfile::tempdir().unwrap();
        // v1 shipped the lang file over a pre-existing copy and backed the original up.
        let lang = dir.path().join(LANG_REL);
        fs::create_dir_all(lang.parent().unwrap()).unwrap();
        fs::write(&lang, "user original").unwrap();
        let zip1 = dir.path().join("v1.zip");
        create_zip(&zip1, &[(MAIN_REL, "main"), (LANG_REL, "modded")]);
        ...
        .unwrap();
        assert_eq!(fs::read_to_string(&lang).unwrap(), "user original");
    ...
    #[test]
    fn uninstall_removes_empty_dirs_the_zip_created_but_keeps_data_lua() {
        let dir = tempfile::tempdir().unwrap();
        write_vanilla_game(dir.path());
        // A zip with a directory entry holding no file; the manifest records files only,
        // so uninstall must sweep it separately.
        let zip_path = dir.path().join("release.zip");
    ...
    #[test]
    fn uninstall_keeps_files_the_mod_dir_gained_since() {
        let dir = tempfile::tempdir().unwrap();
        write_vanilla_game(dir.path());
        let zip_path = dir.path().join("release.zip");
        create_zip(&zip_path, &[(MAIN_REL, "main")]);
        let manifest = install_from_zip(
            &zip_path,
            dir.path(),
            &GameSource::Manual,
        ...
        uninstall::uninstall(dir.path(), &manifest).unwrap();
        assert!(!dir.path().join(MAIN_REL).exists());
