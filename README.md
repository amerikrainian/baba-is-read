...
| [, ] | Previous and next reading category: objects, rules, markers, all (levels, rules, all on the map); empty ones skipped |
| Shift+Period, Shift+Comma | In a level: next and previous kind within the category (all kinds, then each name present, with its count); period and comma then move within that kind |
